...
/leech {magnet link} - To start a download
...
Don't sent any file to the bot or else you will get banned!!
...
function bot(torrent, bot) {
  ...
  bot.on("message", async msg => {
    ...
    bot.sendMessage(chatId, "<b>This Command got some problems</b>\n We Request your <b>PATIENCE</b>");
    try {
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const onDriveUpload = (torr, url) => bot.sendMessage(from, `GDrive: ${url} \nSupport @Bot_Gram_Developer`);
  ...
  bot.onText(statusRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      ...
      if (!torr.done) {
        mess1 += `Downloaded: ${torr.downloaded}\n`;
        mess1 += `Downloading Speed: ${torr.speed}\n`;
        mess1 += `Dowloading: ${torr.progress}\n\n`;
